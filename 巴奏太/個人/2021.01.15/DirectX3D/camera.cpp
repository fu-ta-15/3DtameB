...
// マクロ定義
//-----------------------------------------------------------------------------
#define CAMERA_DISTANCE (150.0f)													// 視点と注視点までの距離
...
void InitCamera(void)
{
	...
	g_camera.posV = D3DXVECTOR3(0.0f, 200.0f, -100.0f);				// 視点
...
void UpdateCamera(void)
{
	...
	//注視点をモデルに
	g_camera.posRDest.x = pModel->pos.x + sinf(pModel->rot.y) * 10;
	g_camera.posRDest.z = pModel->pos.z + cosf(pModel->rot.y) * 10;

	//視点をモデルからカメラの向きに長さ分だけ
	g_camera.posV.x = pModel->pos.x - sinf(g_camera.rot.y) * CAMERA_DISTANCE;
	g_camera.posV.z = pModel->pos.z - cosf(g_camera.rot.y) * CAMERA_DISTANCE;
