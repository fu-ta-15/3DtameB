...
void UpdatePlayer(void)
{
	...
	/* モーション再生 */
	PlayMotion(g_bMotionStart);

	if (GetKeyboardTrigger(DIK_1) == true) StartMotion(MOTIONTYPE_NEUTRAL);
	else if (GetKeyboardTrigger(DIK_2) == true) StartMotion(MOTIONTYPE_ATTACK);
	else if (GetKeyboardTrigger(DIK_3) == true) StartMotion(MOTIONTYPE_WALK);

	//キャラの回転リセット
	if (GetKeyboardTrigger(DIK_N) == true) ResetMotion(true, false, false, true);
}
...
void PlayMotion(bool bPlayMotion)
{
	if (bPlayMotion == true)
	{
		...
		for (int nCntModel = 0; nCntModel < g_player.nNumModel; nCntModel++)
		{
			...
			g_player.aModel[nCntModel].pos.x = g_playerDefaultKey[nCntModel].fPosX + keyDiff[nCntModel].fPosX * ((float)g_player.nCounterMotion / (float)g_player.aMotionInfo[g_player.motionType].aKeyInfo[g_player.nKey].nFrame);
			g_player.aModel[nCntModel].pos.y = g_playerDefaultKey[nCntModel].fPosY + keyDiff[nCntModel].fPosY * ((float)g_player.nCounterMotion / (float)g_player.aMotionInfo[g_player.motionType].aKeyInfo[g_player.nKey].nFrame);
			g_player.aModel[nCntModel].pos.z = g_playerDefaultKey[nCntModel].fPosZ + keyDiff[nCntModel].fPosZ * ((float)g_player.nCounterMotion / (float)g_player.aMotionInfo[g_player.motionType].aKeyInfo[g_player.nKey].nFrame);
		...
		{//ループしない場合
			...
			//モーションリセット
			ResetMotion(true, true, true, true);
		}
	...
	else if (g_bMotionStart == false)
	{
		//モーションリセット
		ResetMotion(true, true, true, false);
	}
}
...
void ResetMotion(bool bPartsReset, bool bCounterReset, bool bKeyReset, bool bMotionTrig)
{
	if (bPartsReset == true)
	{
		...
		for (int nCnt = 0; nCnt < g_player.nNumModel; nCnt++)
		{
			...
			//g_player.aModel[nCnt].pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		}
	}
...
void StartMotion(MOTIONTYPE motionType)
{
	...
	//モーション開始
	if (g_bMotionStart == false)
	{
		g_bMotionStart = true;
	}
	//既に再生されている場合中断して新たなモーションを最初から
	else if (g_bMotionStart == true)
	{
		ResetMotion(false, true, true, false);
	}
}