...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	while (1)
	{
		...
		{//DirectXの処理
			...
			if ((dwCurrentTime - dwExecLastTime) > (1000 / g_nUpdateSpeed))
			{
...
void DrawText(void)
{
	...
	nCharNum += sprintf(&aStr[nCharNum], "[↑] [→] [←] [↓] モデルの移動\n", NULL);
	nCharNum += sprintf(&aStr[nCharNum], "M: モーション再生 ( %d )\n", pPlayer->nCurrentMotion);
	nCharNum += sprintf(&aStr[nCharNum], "[,] [.] モーション変更\n", NULL);
	nCharNum += sprintf(&aStr[nCharNum], "N: キャラの回転リセット\n", NULL);
